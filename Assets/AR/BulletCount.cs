...
public class BulletCount : MonoBehaviour
{
    ...
    void Update()
    {
        m_countText.text = m_count.ToString(); //弾数をテキストとして表示させる
        Reload();
    }

    /// <summary>Rを押されたときと弾数がゼロになった時に実行される</summary>
    void Reload()
    {
        if (Input.GetKeyDown(KeyCode.R) || m_count == 0) 
        {
            m_timeFlag = true;　
            m_reloadFlag = true;
